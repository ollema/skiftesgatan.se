...
<Dialog.Root bind:open>
	...
	<Dialog.Portal>
		...
		<Dialog.Content
			class="fixed inset-y-0 right-0 z-50 flex w-72 flex-col bg-surface p-6 shadow-lg"
		>
			...
			<nav class="mt-8 flex flex-col gap-5">
				{#each links as link (link.path)}
					<a
						...
						class="text-base text-text-secondary no-underline transition-colors duration-120 hover:text-accent"
						class:text-accent={page.url.pathname.startsWith(link.path)}
					>
						{link.label}
					</a>
				{/each}
